...
try {
  ...
  $statement = $pdo->prepare("update public.users_user set email=:email, is_active=true, password='Invalid' where username=:username and is_active=false");
  $statement->execute(array(':email' => $USER_EMAIL_ADDRESS, ':username' => 'tg_admin'));
}
catch (Exception $e) {
